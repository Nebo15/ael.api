defmodule Ael.Secrets.APITest do
  ...
  test "signes url's with resource name (with content-type)" do
    ...
    secret = create_secret("PUT", bucket, resource_id, resource_name, MIME.from_path(resource_name))
  ...
  def create_secret(action, bucket, resource_id) do
  ...
  def create_secret(action, bucket, resource_id, resource_name, content_type) do
    {:ok, secret} = API.create_secret(%{
      action: action,
      bucket: bucket,
      resource_id: resource_id,
